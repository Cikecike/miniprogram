...
from MySQL.Searcher.Openid_to_Name import Openid_to_Name
...
def getArticle_Detail(conn=None,artic_key=None):
    ...
    # 使用execute()方法执行SQL查询
    cursor.execute("SELECT a.artic_title,b.content,a.author_id,a.submit_time,c.like_sum,c.comments_num FROM article AS a,artic_content AS b,artic_info_active AS c WHERE a.artic_key=%d AND b.artic_key=%d AND c.artic_key=%d;"%(artic_key,artic_key,artic_key))
    # 使用fetchone()方法获取单条数据
    ...
    for d in data:
        ...
        p['Content']=d['content']
        p['Author']=Openid_to_Name(openid=d['author_id'])
